...
def generate_prompt(ctx):
    """
    Prompt v3：v2 + 恢复同类型3条 + 数值锚点，目标拉回±1.0准确率
    测试集得分 93.52（低于 v2 的 95.47）
    """
    ...
    target_tags = short_text(movie.get('tags'), 70)
    target_summary = short_text(movie.get('summary'), 200)
...
预测原则：
- 分数相对于该用户个人标准，须结合均分、最近均分、同类型均分综合判断
- 最近评分与同类型评分是最强信号；无充分理由时，预测应与二者接近（通常相差不超过1）
- 高低分锚点用于判断目标片接近上限还是下限
- 无历史时参考跨用户示例分数区间，勿机械默认3分
...
- 回复有且仅有一行，格式为：[Result:X]（X为1-5的整数）"""
    ...
    if cold_start:
        ...
        ref_avg_line = f"参考均分约{ref_avg:.1f}" if ref_avg is not None else ''
        system_prompt += "\n\n冷启动：结合参考示例分数区间与目标电影特征；勿因动作/名导等盲目给高分。"
...
[Result:"""
        ...
        if avg <= 3.2:
            cal = "偏严格用户：3分=中性，2分=不满，4分=很喜欢。"
        elif avg >= 4.0:
            cal = "偏宽松用户：4分常见，3分=一般，2分=强烈不喜欢。"
        else:
            cal = "居中用户：对照均分与锚点，3/4分边界需细辨。"
        system_prompt += f"\n\n{cal}"
        ...
        similar_text = ctx.format_history_list(similar, 'compact') if similar else '无'

        taste_lines = []
        if highest:
            hi = highest[0]
            taste_lines.append(
                f"上限{movie_title(hi)}({hi.get('rating')}分): {short_text(hi.get('comment'), 32)}"
            )
        if lowest:
            lo = lowest[0]
            if not highest or lo.get('movie_id') != highest[0].get('movie_id'):
                taste_lines.append(
                    f"下限{movie_title(lo)}({lo.get('rating')}分): {short_text(lo.get('comment'), 32)}"
                )
        taste_text = ' | '.join(taste_lines) if taste_lines else '无'

        anchor_parts = [f"均分{avg:.1f}"]
        if recent_avg is not None:
            anchor_parts.append(f"最近{recent_avg:.1f}")
        if sim_avg is not None:
            anchor_parts.append(f"同类型{sim_avg:.1f}")
        if mode is not None:
            anchor_parts.append(f"众数{mode}分")
        anchor_line = ' | '.join(anchor_parts)

        user_prompt = f"""## 用户画像
{anchor_line} | 区间{stats['min']}-{stats['max']} | {format_dist(dist)}
