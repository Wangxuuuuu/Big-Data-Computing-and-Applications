def generate_prompt(ctx):
    """
    Prompt v10：在 v8 基础上对高频泛标签轻度降权。
    目标：保留相似历史的完整覆盖，仅在排序时优先更具体的标签证据。
    测试集 #1043（2026-06-23）：最终得分 96.80（失败，低于 v8）
    """
    ...
    def custom_similar(history, target_tags, target_director, n=2):
        generic_tags = {
            '美国', '剧情', '科幻', '喜剧', '爱情', '动作', '悬疑', '人性',
            '日本', '惊悚', '美国电影', '动画', '犯罪', '电影', '中国大陆'
        }
        scored = []
        for idx, item in enumerate(history):
            item_tags = parse_tags(item.get('tags'))
            common = overlap_tags(item_tags, target_tags)
            score = sum(7 if tag in generic_tags else 10 for tag in common)
